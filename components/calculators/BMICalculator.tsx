...
export default function BMICalculator() {
  ...
  // ── existing calculate function (unchanged) ──
  const calculate = () => {
  ...
  return (
    <>
      ...
      {/* ── Calculator Grid (design + logic 100% unchanged) ── */}
        ...
        <div className="space-y-2">
          {FAQ_DATA.map((item, i) => (
            ...
            >
              ...
              {/*
                Answer is ALWAYS in the DOM for Google to crawl.
                max-h-0 visually collapses it, but the text remains in HTML source.
              */}
              <div
                id={`faq-answer-${i}`}
                role="region"
              ...
              {/* Hidden fallback for SEO — ensures answer text is always indexable */}
              {openFaq !== i && (
                <span className="sr-only" itemProp="text" aria-hidden="true">
                  {item.a}
                </span>
              )}
            </div>
