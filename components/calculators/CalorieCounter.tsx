...
const WEBAPP_SCHEMA = JSON.stringify({
    ...
    url: "https://www.numrexo.com/health/calorie-counter",
...
const BREADCRUMB_SCHEMA = JSON.stringify({
    ...
    itemListElement: [
        ...
        { "@type": "ListItem", position: 3, name: "Calorie Counter", item: "https://www.numrexo.com/health/calorie-counter" },
    ],
});
...
export default function CalorieCounter() {
    ...
    return (
        <>
                    ...
                    <div className="p-6 space-y-3 max-h-96 overflow-y-auto">
                        {foods.map((food, i) => (
                            <div key={i} className="flex flex-wrap gap-2 items-center border-b border-gray-800 pb-3">
                                ...
                                <div className="w-20 relative"><input type="number" placeholder="Cal" value={food.calories} onChange={(e) => updateFood(i, "calories", e.target.value)} className="w-full px-2 py-2 bg-[#0f1525] border border-gray-700 rounded-lg text-white text-sm" /><span className="absolute right-2 top-1/2 -translate-y-1/2 text-xs text-gray-500">cal</span></div>
                                <div className="w-20 relative"><input type="number" step="0.1" placeholder="P" value={food.protein} onChange={(e) => updateFood(i, "protein", e.target.value)} className="w-full px-2 py-2 bg-[#0f1525] border border-gray-700 rounded-lg text-white text-sm" /><span className="absolute right-2 top-1/2 -translate-y-1/2 text-xs text-gray-500">g</span></div>
                                <div className="w-20 relative"><input type="number" step="0.1" placeholder="C" value={food.carbs} onChange={(e) => updateFood(i, "carbs", e.target.value)} className="w-full px-2 py-2 bg-[#0f1525] border border-gray-700 rounded-lg text-white text-sm" /><span className="absolute right-2 top-1/2 -translate-y-1/2 text-xs text-gray-500">g</span></div>
                                <div className="w-20 relative"><input type="number" step="0.1" placeholder="F" value={food.fat} onChange={(e) => updateFood(i, "fat", e.target.value)} className="w-full px-2 py-2 bg-[#0f1525] border border-gray-700 rounded-lg text-white text-sm" /><span className="absolute right-2 top-1/2 -translate-y-1/2 text-xs text-gray-500">g</span></div>
                                {foods.length > 1 && <button onClick={() => removeFood(i)} className="px-2 py-2 text-red-400 hover:text-red-300">✕</button>}
                            </div>
                        ))}
                    </div>
                    <div className="px-6 py-4 border-t border-gray-800">
                        <button onClick={calculate} className="w-full py-2 rounded-lg bg-gradient-to-r from-orange-500 to-orange-700 text-white font-semibold hover:shadow-lg transition-all">Calculate Total →</button>
                    </div>
                </div>
            ...
            <section className="mb-8"><h2 className="text-xl font-semibold text-white mb-3">About Calorie Counter</h2><p className="text-gray-400 text-sm leading-relaxed">Track your daily calorie and macronutrient intake. Add foods manually or use our quick-add database. Perfect for weight loss, muscle gain, or maintaining a healthy lifestyle.</p></section>
