...
export default function IdealWeightCalculator() {
    ...
    const calculate = () => {
        ...
        const toLbs = (kg: number) => kg;
        const toKg = (kg: number) => kg;
        ...
        <>
            ...
            <nav aria-label="Breadcrumb" className="mb-5">
                <ol className="flex flex-wrap items-center gap-1.5 text-xs text-gray-500" itemScope itemType="https://schema.org/BreadcrumbList">
                    ...
                    <li itemScope itemProp="itemListElement" itemType="https://schema.org/ListItem">
                        <a href="https://www.numrexo.com/health" itemProp="item" className="hover:text-gray-300">Health Calculators</a>
            ...
            <section className="mb-8">
                ...
                <div className="bg-[#111827] border border-gray-800 rounded-xl overflow-hidden">
                    <table className="w-full text-sm">
                        ...
                        <tbody>
                            {FRAME_SIZES.map((row, i) => (<tr key={i} className="border-b border-gray-800/50 hover:bg-white/5"><td className="py-3 px-4 text-gray-300">{row.frame}</td><td className="py-3 px-4 text-gray-300">{row.wristMen}</td><td className="py-3 px-4 text-gray-300">{row.wristWomen}</td><td className="py-3 px-4 text-gray-300">{row.frame === "small" ? "-10%" : row.frame === "large" ? "+10%" : "No adjustment"}</td></tr>))}
                        </tbody>
                    </table>
                </div>
            </section>
