...
export default function PaintCalculator() {
    ...
    return (
        <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                <div className="bg-[#111827] border border-gray-800 rounded-xl overflow-hidden">
                    ...
                    <div className="p-6 space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <div><label className="block text-xs font-semibold text-gray-400 mb-2">Paint Type</label><select value={paintType} onChange={(e) => setPaintType(e.target.value as any)} className="w-full px-4 py-3 bg-[#0f1525] border border-gray-700 rounded-lg text-white"><option value="paint">Wall Paint</option><option value="primer">Primer</option><option value="enamel">Enamel Paint</option></select></div>
                            <div><label className="block text-xs font-semibold text-gray-400 mb-2">Number of Coats</label><input type="number" step="0.5" placeholder="2" value={coats} onChange={(e) => setCoats(e.target.value)} className="w-full px-4 py-3 bg-[#0f1525] border border-gray-700 rounded-lg text-white" /></div>
                        </div>
                        <div className="flex items-center gap-3">
                            <input type="checkbox" id="includeCeiling" checked={includeCeiling} onChange={(e) => setIncludeCeiling(e.target.checked)} className="w-4 h-4" />
                            <label htmlFor="includeCeiling" className="text-sm text-gray-300">Include Ceiling</label>
                        </div>
                        <div><label className="block text-xs font-semibold text-gray-400 mb-2">Price per Liter (₹) - Optional</label><input type="number" step="10" placeholder="250" value={pricePerLiter} onChange={(e) => setPricePerLiter(e.target.value)} className="w-full px-4 py-3 bg-[#0f1525] border border-gray-700 rounded-lg text-white" /></div>
                        <button onClick={calculate} className="w-full py-3 rounded-lg bg-gradient-to-r from-blue-500 to-blue-700 text-white font-semibold hover:shadow-lg">Calculate Paint →</button>
                    </div>
                </div>
            ...
            <section className="mb-8"><h2 className="text-xl font-semibold text-white mb-3">About Paint Calculator</h2><p className="text-gray-400 text-sm leading-relaxed">Estimate how much paint you need for your room. Includes walls, ceiling, doors, windows, and multiple coats. Perfect for home renovation and painting projects.</p></section>

            <section className="mb-8"><h2 className="text-xl font-semibold text-white mb-4">Paint Coverage Guide</h2>
                <div className="bg-[#111827] border border-gray-800 rounded-xl overflow-hidden">
